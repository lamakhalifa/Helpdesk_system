...
class AuthController extends Controller
{
    ...
    public function resetPassword(Request $request){
        ...
        $url_token = $user->createToken('urlToken')->accessToken;
        Token::create([
    ...
    public function checkToken(Request $request){
        $TokenInfo= Token::where('url_token', $request->url_token)->first();
        if($TokenInfo) {
            $validatedData = $request->validate([
            ...
            if ( $validatedData && $TokenInfo->expires_at >= now()) {
                $update= User::where('email', $TokenInfo->email)->first()->update(['password' => $validatedData['password']]);
                if($update){
                    $record = Token::find($TokenInfo->id);
                    $record->delete();
                }
            }else{
